...
int store_pixels(char *filename, animated_gif *image) {
    ...
    printf("[DEBUG] Background color (%d,%d,%d) -> (%d,%d,%d)\n",
           ...
           image->g->SColorMap->Colors[image->g->SBackGroundColor].Green,
           image->g->SColorMap->Colors[image->g->SBackGroundColor].Blue, moy,
           moy, moy);
#endif
...
#if SOBELF_DEBUG
                printf(
                    "[DEBUG] Transparency color image %d (%d,%d,%d) -> "
                    "(%d,%d,%d)\n",
    ...
    for (i = 0; i < image->n_images; i++) {
        for (j = 0; j < image->g->SavedImages[i].ExtensionBlockCount; j++) {
            ...
            if (f == GRAPHICS_EXT_FUNC_CODE) {
                int tr_color =
                    image->g->SavedImages[i].ExtensionBlocks[j].Bytes[3];
                ...
                if (tr_color >= 0 && tr_color < 255) {
                    ...
                    printf(
                        ...
                        i, image->g->SColorMap->Colors[tr_color].Red,
                        image->g->SColorMap->Colors[tr_color].Green,
                        image->g->SColorMap->Colors[tr_color].Blue, moy, moy,
                        moy);
#endif
                    ...
                    if (found == -1) {
                        if (n_colors >= 256) {
                            fprintf(stderr,
                                    "Error: Found too many colors inside the "
                                    "image\n");
                            return 1;
                        }
    ...
    if (cmo == NULL) {
        fprintf(stderr,
                "Error while creating a ColorMapObject w/ %d color(s)\n",
                n_colors);
        return 1;
    }
