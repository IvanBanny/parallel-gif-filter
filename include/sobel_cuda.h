...
static void allocate_buffers(size_t max_pixel, pixel **h_buf,
...
static void send_data_to_gpu(animated_gif *image, int index, pixel *h_buf,
                      pixel *d_unpacked_pixels, packed_pixel *color_image_gpu,
                      cudaStream_t stream) {
...
static void apply_grey_filter_cuda(animated_gif *image, int index,
...
static void apply_blur_filter_cuda(animated_gif *image, int index, int size, int threshold,
...
static void apply_sobel_filter_cuda(animated_gif *image, int index, grey_pixel *d_grey,
...
static void fetch_data_from_gpu(animated_gif *image, int index, pixel *h_buf,
...
void sobel_cuda(animated_gif* image)
{
    size_t max_pixels = 0;
    ...
    for (int i = 0; i < image->n_images; ++i) {
        ...
        apply_grey_filter_cuda(image, i, d_color[s], d_grey1[s], streams[s]);
        apply_blur_filter_cuda(image, i, 5, 20, h_end_flag, d_end_flag, d_grey1[s],
                          d_grey2[s], streams[s]);
        apply_sobel_filter_cuda(image, i, d_grey1[s], d_color[s], streams[s]);
